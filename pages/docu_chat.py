                            ...
                            children=[
                                ...
                                html.Div(
                                    ...
                                    style={
                                        ...
                                        "color": "white" ,
                                         "marginTop":"400px" # Change text color to white if needed
                                    }
                                )
                            ],
                            className="flex-fill",
...
@callback(
    Output("sidebar-docu-chat", "className"),
    Output("sidebar-toggle-button-container-docu-chat", "style"),
    Output("sidebar-toggle-button-container-docu-chat", "children"),
    Output("sidebar-content-docu-chat", "style"),
    Output("sidebar-content-docu-chat-2", "style"),
    Output("contract-heading-docu-chat", "style"),
    Output("docu-chat-heading", "style"),
    Output("docu-chat-text", "style"),
    Output("docu-chat-content", "style"),
    Input("sidebar-toggle-button-container-docu-chat", "n_clicks"),
...
@callback(
    ...
    [Input("up-icon", "n_clicks"), Input("down-icon", "n_clicks")],
    [State("number", "children")],
)
def update_number(up_clicks, down_clicks, current_value):
    if callback_context.triggered:
        triggered_id = callback_context.triggered[0]["prop_id"].split(".")[0]
        current_number = int(current_value.split("/")[0])
        total_number = int(current_value.split("/")[1])
        if triggered_id == "up-icon":
            new_number = min(current_number + 1, total_number)
        else:
            new_number = max(current_number - 1, 1)
        return f"{new_number}/{total_number}"
    else:
